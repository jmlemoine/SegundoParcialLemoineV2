...
var baseDeDatosAbierta = baseDeDatosIndexed.open("SPLemoine", 1);
...
function agregarDelFormulario() {
    ...
    function agregarConLatitudyLongitud(ubicacion) {
        ...
        procesamientoDeDatos.oncomplete = function (e) {
            ...
            alert('Persona ingresada al mapa.');
        };
...
function imprimirTabla(registrosPersonas) {
    ...
    for (var key in registrosPersonas) {
        ...
        filaAListar.insertCell().innerHTML = "<button class='btn btn-light' onclick='borrarPersona(" + registrosPersonas[key].identificador + ")'><i class='fas fa-trash-alt'/> Borrar </button>";
        filaAListar.insertCell().innerHTML = "<button class='btn btn-light' onclick='modificarPersona(" + registrosPersonas[key].identificador + ")'><i class='far fa-edit'/> Modificar</button>";
    }
