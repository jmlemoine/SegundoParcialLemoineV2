...
public class Enrutamiento {
    public static void crearRutas(){
        ...
        get("/agregarRegistro", (req, res) -> {
            StringWriter writer = new StringWriter();
            Map<String, Object> atributos = new HashMap<>();
            Template template = configuration.getTemplate("plantillas/agregarRegistro.ftl");
        ...
        get("/mapa", (req, res) -> {
            StringWriter writer = new StringWriter();
            Map<String, Object> atributos = new HashMap<>();
            Template template = configuration.getTemplate("plantillas/mapa.ftl");
            List<Persona> personas = PersonaServicio.getInstancia().listar();
        ...
        post("/sincronizar", (req, res) -> {
            ...
            for (int i = 0; i < arregloAux.size(); i++) {
                if ((i + 1) % 6 == 0) {
                    ...
                    PersonaServicio.getInstancia().crear(personaNueva);
                }
            }
